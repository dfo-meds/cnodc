...
import cnodc.units.parsing as unp
import cnodc.units.units as un

from cnodc.units.structures import Integer
...
class TestLowLevel(ut.TestCase):
    ...
    def test_parse_literal(self):
        for case in TestLowLevel.INTEGERS:
            with self.subTest(case=case):
                self.assertIsInstance(unp._parse_literal(case), Integer)
    ...
    def test_literal_components(self):
        for case in TestLowLevel.INTEGERS:
            with self.subTest(case=case):
                self.assertEqual(unp._parse_unit_for_exponents(case), Integer(case))
    ...
    def test_exponent_component(self):
        ...
        for in_value, base_unit, exponent in tests:
            with self.subTest(input=in_value):
                ...
                self.assertEqual(x.exponent, Integer(exponent))
