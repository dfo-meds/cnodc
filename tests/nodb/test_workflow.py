...
class TestWorkflow(BaseTestCase):
    ...
    def test_config_problems(self):
        tests = [
            ...
            ({'label': {'und': '2'}, 'additional_targets': [{'directory': str(self.temp_dir), 'metadata': {'foobar': 'foobar'}}]}, None, 'good metadata for additional_target[0]'),
            ({'label': {'und': '2'}, 'working_target': {'directory': str(self.temp_dir)}, 'steps': 'foobar'}, 'NODB-VALIDATION-2010', 'bad processing steps'),
            ({'label': {'und': '2'}, 'working_target': {'directory': str(self.temp_dir)}, 'steps': {'key': 'bar'}}, 'NODB-VALIDATION-2011', 'step must be a dict'),
            ({'label': {'und': '2'}, 'working_target': {'directory': str(self.temp_dir)}, 'steps': {'key': {'foo': 'bar'}}}, 'NODB-VALIDATION-2012', 'step must have an order'),
            ({'label': {'und': '2'}, 'working_target': {'directory': str(self.temp_dir)}, 'steps': {'key': {'order': 'bar'}}}, 'NODB-VALIDATION-2013', 'order must be an int'),
            ({'label': {'und': '2'}, 'working_target': {'directory': str(self.temp_dir)}, 'steps': {'key': {'order': 1, 'name': '1'}, 'key2': {'order': 1, 'name': '2'}}}, 'NODB-VALIDATION-2017', 'duplicate orders'),
            ({'label': {'und': '2'}, 'working_target': {'directory': str(self.temp_dir)}, 'steps': {'key': {'order': 2}}}, 'NODB-VALIDATION-2014', 'step must have a name'),
            ({'label': {'und': '2'}, 'working_target': {'directory': str(self.temp_dir)}, 'steps': {'key': {'order': 2, 'name': ''}}}, 'NODB-VALIDATION-2014', 'name cannot be blank'),
            ({'label': {'und': '2'}, 'working_target': {'directory': str(self.temp_dir)}, 'steps': {'key': {'order': 2, 'name': 'foobar', 'priority': '1e13qwewsd'}}}, 'NODB-VALIDATION-2015', 'priority must be a number'),
            ({'label': {'und': '2'}, 'working_target': {'directory': str(self.temp_dir)}, 'steps': {'key': {'order': 2, 'name': 'foobar', 'priority': '5'}}}, None, 'good step definition'),
