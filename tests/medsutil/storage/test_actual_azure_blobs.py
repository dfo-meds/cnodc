...
class TestLiveAzureBlobs(BaseTestCase):
    ...
    def test_properties(self):
        with AzureBlobHandle.build(f'{self.base_azure_share}test.txt') as file:
            self.assertTrue(file.supports_tiering())
            self.assertTrue(file.supports_metadata())

    def test_set_metadata(self):
        with AzureBlobHandle.build(f'{self.base_azure_share}new.txt') as file:
            try:
                file.upload([b'12345'])
    ...
    def test_delete_dir(self):
        with AzureBlobHandle.build(f'{self.base_azure_share}test_delete_dir/') as handle:
    ...
    def test_non_existing_file(self):
        with AzureBlobHandle.build(f'{self.base_azure_share}test3.txt') as file:
            self.assertFalse(file.exists())
            self.assertFalse(file.is_dir())
    ...
    def test_dir(self):
        with AzureBlobHandle.build(f'{self.base_azure_share}test/foo/bar/') as file:
            self.assertTrue(file.exists())
    ...
    def test_walk_in_memory(self):
        with AzureBlobHandle.build(f'{self.base_azure_share}/') as blob:
