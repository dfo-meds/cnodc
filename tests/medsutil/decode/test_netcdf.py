...
class TestNetCDFCommonDecode(BaseTestCase):
    ...
    def test_good_basic_config(self):
        ...
        with nc.Dataset("inmemory.nc", "w", diskless=True) as ds:
            ...
            with self.subTest(msg="explicit variable"):
                self.assertIn('var:test', map_info['record_vars'])
                self.assertNotIn('var:test', map_info['global_vars'])

            with self.subTest(msg="explicit attribute"):
                self.assertIn('attribute:test2', map_info['global_vars'])
                self.assertNotIn('attribute:test2', map_info['record_vars'])
                self.assertEqual(map_info['global_vars']['attribute:test2'].source, 'test2')
                self.assertEqual(map_info['global_vars']['attribute:test2'].mapping_type, 'attribute')
                self.assertEqual(map_info['global_vars']['attribute:test2'].data_map, {'1': 'one', '2': 'two'})

            with self.subTest(msg="explicit global variable"):
                self.assertNotIn('globalvar:test3', map_info['record_vars'])
                self.assertIn('globalvar:test3', map_info['global_vars'])
                self.assertEqual(map_info['global_vars']['globalvar:test3'].source, 'test3')
                self.assertEqual(map_info['global_vars']['globalvar:test3'].mapping_type, 'globalvar')

            with self.subTest(msg="implicit variable"):
                self.assertIn('test4', map_info['record_vars'])
                self.assertNotIn('test4', map_info['global_vars'])
                self.assertEqual(map_info['record_vars']['test4'].source, 'test4')
                self.assertEqual(map_info['record_vars']['test4'].mapping_type, 'var')

            expected = {
                'mapping_type': 'var',
                'source': 'test',
                'targets': ['metadata/bar'],
                'is_index': True
            }
            for key in expected:
                with self.subTest(expected_key=key):
                    self.assertEqual(expected[key], getattr(map_info['record_vars']['var:test'], key))
    ...
    def test_good_basic_config_explicit(self):
        with nc.Dataset("inmemory.nc", "w", diskless=True) as ds:
            ...
            map_info = mapper._get_ocproc2_map()
            with self.subTest(msg="Implicit variable"):
                self.assertIn('1', map_info['record_vars'])
                self.assertNotIn('1', map_info['global_vars'])
                self.assertEqual(map_info['record_vars']['1'].source, 'test')
                self.assertEqual(map_info['record_vars']['1'].mapping_type, 'var')
            with self.subTest(msg="Explicit attribute"):
                self.assertIn('2', map_info['global_vars'])
                self.assertNotIn('2', map_info['record_vars'])
                self.assertEqual(map_info['global_vars']['2'].source, 'test2')
                self.assertEqual(map_info['global_vars']['2'].mapping_type, 'attribute')
            with self.subTest(msg="Explicit global variable"):
                self.assertIn('3', map_info['global_vars'])
                self.assertNotIn('3', map_info['record_vars'])
                self.assertEqual(map_info['global_vars']['3'].source, 'test3')
                self.assertEqual(map_info['global_vars']['3'].mapping_type, 'globalvar')
            with self.subTest(msg="Explicit variable"):
                self.assertIn('4', map_info['record_vars'])
                self.assertNotIn('4', map_info['global_vars'])
                self.assertEqual(map_info['record_vars']['4'].source, 'test4')
                self.assertEqual(map_info['record_vars']['4'].mapping_type, 'var')
    ...
    def test_global_variable_str_mapping(self):
        with nc.Dataset("inmemory.nc", "r+", diskless=True) as ds:
            ...
            values = [1, 2, 3, 4, 5]
            ds.variables['test'][:] = values
            ds.variables['test2'][:] = netcdf_string_to_bytes('hello world', 16)
            mapper = NetCDFCommonMapper(ds, {
                'ocproc2_map': {
                    'test': {
                        'target': 'metadata/Bar',
    ...
    def test_global_variable_vlen_str_mapping(self):
        with nc.Dataset("inmemory.nc", "r+", diskless=True) as ds:
            ...
            values = [1, 2, 3, 4, 5]
            ds.variables['test'][:] = values
            ds.variables['test2'][:] = netcdf_string_to_vlen_bytes('hello world')
            mapper = NetCDFCommonMapper(ds, {
                'ocproc2_map': {
                    'test': {
                        'target': 'metadata/Bar',
        ...
        with nc.Dataset("inmemory.nc", "r+", diskless=True) as ds:
            ...
            for idx, record in enumerate(records):
                with self.subTest(record_no=idx):
                    self.assertIn('Bar2', record.metadata)
                    self.assertIsInstance(record.metadata['Bar2'], MultiElement)
                    self.assertIsInstance(record.metadata['Bar2'].value, list)
