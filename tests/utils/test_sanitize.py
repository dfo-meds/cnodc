...
class TestDataSanitization(BaseTestCase):
    ...
    def test_json_cleaning(self):
        ...
        for in_value, expected in tests:
            with self.subTest(input=in_value, expected=expected):
                self.assertEqual(expected, clean_for_json(in_value))
    ...
    def test_nfc_normalization(self):
        ...
        for c in cnodc.util.sanitize.UNICODE_SPACES:
            tests.append((f"a{c}b", "a b"))
        for c in cnodc.util.sanitize.UNICODE_DASHES:
            tests.append((f"a{c}b", "a-b"))
        for in_value, expected in tests:
            with self.subTest(input=in_value, expected=expected):
                self.assertEqual(expected, normalize_string(in_value))
