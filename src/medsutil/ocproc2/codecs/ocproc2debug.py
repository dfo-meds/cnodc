...
class OCProc2DebugCodec(BaseCodec):
    ...
    def _encode_profile(self, record_type, record_idx, rs: ocproc2.RecordSet):
        ...
        if rs.records:
            ...
            m_common: dict[str, AbstractElement] = {x: rs.records[0].metadata[x] for x in rs.records[0].metadata.keys()}
            ...
            if c_common or p_common or m_common:
                ...
                for mn in sorted(m_common.keys()):
                    s += f'      {mn} = {self._encode_element(mn, m_common[mn], "", skip_name=True)}\n'
                for cn in c_keys:
                    if cn not in c_common:
                        continue
                    for mn in sorted(c_common[cn].keys()):
                        s += f'      {cn}[{mn}] = {self._encode_element(str(mn), c_common[cn][mn], "", skip_name=True)}\n'
                for pn in p_keys:
                    if pn not in p_common:
                        continue
                    for mn in sorted(p_common[pn]):
                        s += f'      {pn}[{mn}] = {self._encode_element(str(mn), p_common[pn][mn], "", skip_name=True)}\n'
                if add_note:
                    s += f'      ***** indicates sub-records not shown on this output'
        s += '\n'
        return s

    def _build_table(self, rs, group: str) -> tuple[dict, dict, dict, dict]:
        c_keys = set()
        for sr in rs.records:
    ...
    def _encode_element(self, name: str, v: ocproc2.AbstractElement, prefix='    ', ignore_parameters: t.Optional[list] = None, skip_name: bool = False):
        ...
        if isinstance(v, ocproc2.MultiElement):
            ...
            if v.value is None:
                ...
                unc = v.metadata['Uncertainty'].to_float() or 0
