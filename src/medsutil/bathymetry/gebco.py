...
from medsutil.adecimal import AccurateDecimal
...
class GEBCO2023BathymetryModel(BathymetryModel):
    ...
    def __init__(self, base_dir: str, uncertainty: float):
        ...
        self._gebco_dir = pathlib.Path(base_dir)  # self.config.as_path('gebco2023_directory')
    ...
    def water_depth(self, x: amath.AnyNumber, y: amath.AnyNumber) -> t.Optional[amath.AnyNumber]:
        min_x, max_x = amath.min_max_range(x)
        min_y, max_y = amath.min_max_range(y)
        ...
        if min_s is not None and max_s is not None:
            ...
            # the lowest possible value to the highest possible value (assuming uniform distribution)
            return AccurateDecimal(midpoint_s, (max_s - midpoint_s) / amath.sqrt(3))
        else:
            # The result is on land, we don't really care about the elevation,
            # so return something clearly above 0
            return AccurateDecimal(100, 0)
    ...
    def _identify_cell(self, x: int | float | decimal.Decimal, y: int | float | decimal.Decimal) -> tuple[int, int]:
