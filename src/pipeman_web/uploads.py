...
class UploadError(CodedError): CODE_SPACE = 'UPLOADCTRL'
...
def list_all_workflows_with_access(nodb: interface.NODB, login: LoginController) -> t.Iterable[str]:
    user_perms = login.current_permissions()
    with nodb as db:
        for workflow in NODBUploadWorkflow.find_all(db):
            if workflow.workflow_name is not None and workflow.configuration is not None and workflow.configuration.check_access(user_perms):
...
class UploadController:
    ...
    def _check_access(self, workflow: NODBUploadWorkflow):
        ...
        return workflow.configuration is not None and workflow.configuration.check_access(user_permissions)
    ...
    def _request_directory(self) -> pathlib.Path:
        if self._request_dir is None:
            if not flask.current_app.config.get('UPLOAD_FOLDER'):
                raise UploadError("No upload folder configured", 1000)
            upload_dir = pathlib.Path(flask.current_app.config['UPLOAD_FOLDER']).resolve()
            try:
                self._request_dir = upload_dir / 'requests' / self.workflow_name / self._ensure_request_id()
                self._request_dir.mkdir(0o660, parents=True, exist_ok=True)
                if not self._request_dir.is_dir():
                    self._request_dir = None
                    raise UploadError("Could not make a request directory, one of the paths is a file", 1001)
            except OSError as ex:
                self._request_dir = None
                raise UploadError("Could not make a request directory", 1002) from ex
        return self._request_dir

    def _check_token(self, allow_new: bool = False) -> bool:
        if self.token is None:
            if not allow_new:
                raise UploadError('No continuation token found in request', 1003)
            return True
        token_file = self._request_directory() / ".token"
        if not token_file.exists():
            raise UploadError('No continuation token found on the server', 1004)
        with open(token_file, "rb") as h:
            token_hash = h.read()
            if not secrets.compare_digest(token_hash, UploadController.hash_token(self.token)):
                raise UploadError('Invalid continuation token', 1005)
        return True
    ...
    def _check_data_integrity(self, data: bytes, headers: dict[str, str]):
        if 'x-cnodc-upload-md5' in headers:
            ...
            md5_actual = hashlib.md5(data, usedforsecurity=False).hexdigest().lower()
            if md5_sent != md5_actual:
                raise UploadError(f'MD5 mismatch [{md5_sent}] vs [{md5_actual}]', 1006)
    ...
    def _save_data(self, data: bytes, max_size: int = None):
        ...
        if max_size is not None and (total_size + len(data)) > max_size:
            raise UploadError(f"Maximum size of {max_size} exceeded", 1007)
    ...
    def get_working_file(self) -> pathlib.Path:
        if self._assembled_file is None:
            ...
            while bin_file.exists():
                ...
                bin_file = request_dir / f"part.{idx}.bin"
            if not files:
                raise UploadError(f"No files found in request directory", 1008)
            if len(files) == 1:
    ...
    def upload_request(self, data: bytes, headers: dict[str, str]) -> UploadResult:
        ...
        with self.nodb as db:
            workflow = self._load_workflow(db)
            if workflow.workflow_name is None or workflow.configuration is None:
                raise UploadError('Invalid workflow', 1030)
            working_headers = self._save_metadata(headers)
            self._save_data(data, workflow.configuration.max_file_size)
            if 'x-cnodc-more-data' in headers and headers['x-cnodc-more-data'] == '1':
                ...
                metadata: dict[str, str] = {
                    'source': 'web_uploader',
                    'correlation_id': self.request_id or '',
                    'user': user.username if user else '__anonymous'
                }
