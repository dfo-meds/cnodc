...
from cnodc.science.adecimal import AccurateDecimal, NonAccurateNumber
...
def with_minimum_uncertainty(num: AnyNumber, uncertainty: NonAccurateNumber):
    if isinstance(num, AccurateDecimal):
        if uncertainty > num.std_dev:
            return AccurateDecimal(num.num, uncertainty)
        return num
    elif isinstance(num, UFloat):
        if uncertainty > num.std_dev:
            return ufloat(num.nominal_value, uncertainty)
        return num
    else:
        return AccurateDecimal(num, uncertainty)
