    ...
    def _on_data_load(self):
        ...
        if self._record_metadata is None:
            ...
            if self.has_variable('PLATFORM_TYPE'):
                info = self._data['glider_model_map'][self.var_to_string('PLATFORM_TYPE').lower()]
            ...
            if self.has_attribute('platform_name') and self.has_variable('DEPLOYMENT_START_TIME'):
                self._record_metadata['metadata/CruiseID'] = f"{self._dataset.getncattr('platform_name')}_{self.var_to_string('DEPLOYMENT_START_TIME')[0:8]}"
