...
class GTSPPInitialQualityFlagsCheck(BaseTestSuite):
    ...
    def _set_quality_flags(self, record, context: TestContext):
        ...
        self.set_all_quality_flags(record, context, use_qc)

    def set_all_quality_flags(self, record: ocproc2.DataRecord, context: TestContext, use_qc: bool):
        for key in record.parameters:
            with context.parameter_context(key) as ctx2:
                self.test_all_subvalues(ctx2, self._set_flags_on_element, use_qc=use_qc)
        ...
        for key in record.coordinates:
            with context.coordinate_context(key) as ctx2:
                self.test_all_subvalues(ctx2, self._set_flags_on_element, use_qc=use_qc)
        self.test_all_subrecords(context, self.set_all_quality_flags, use_qc=use_qc)
