...
class OCProc2BinCodec(BaseCodec):
    ...
    def _get_codec(self, codec: str) -> BaseCodec:
        if codec == 'JSON':
            from cnodc.ocproc2.codecs.ocproc2json import OCProc2JsonCodec
            return OCProc2JsonCodec(halt_flag=self._halt_flag)
        elif codec == 'YAML':
            from cnodc.ocproc2.codecs.ocproc2yaml import OCProc2YamlCodec
            return OCProc2YamlCodec(halt_flag=self._halt_flag)
        elif codec == 'PICKLE':
            from cnodc.ocproc2.codecs.ocproc2pickle import OCProc2PickleCodec
