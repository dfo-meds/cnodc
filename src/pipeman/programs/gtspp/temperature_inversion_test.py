...
class GTSPPTemperatureInversionTest(BaseTestSuite):
                ...
                if self._check_for_temperature_inversion(ldt1, ldt2, ldt, ref):
                    for i in range(ldt2[0], len(recordset.records)):
                        ...
                        for av in recordset.records[i].parameters['Temperature'].all_values():
                            if self.should_test_value(av, raise_ex=False):
    ...
    def _check_for_temperature_inversion(self,
        ...
        for idx in range(0, min(len(ldt1[2]), len(ldt2[2]), len(ldt3[2]))):
            ...
            t13_avg = (t1 + t3) / 2.0
            diff = t2 - t13_avg
            if medsutil.amath.is_greater_than(diff, self._maxima_threshold):
                ref['maxima'].append(ldt2[1])
                return ref['minima'] and any(
                    medsutil.amath.is_greater_than(x, ldt2[1]) and medsutil.amath.is_less_than(x - ldt2[1], self._depth_gap) for x in ref['minima'])
            elif medsutil.amath.is_less_than(diff, self._minima_threshold):
                ref['minima'].append(ldt2[1])
                return ref['maxima'] and any(
                    medsutil.amath.is_greater_than(x, ldt2[1]) and medsutil.amath.is_less_than(x - ldt2[1], self._depth_gap) for x in ref['maxima'])
        return False
