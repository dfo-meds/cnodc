...
class GTSPPSpikeGradientTest(BaseTestSuite):
    ...
    def _run_extrema_spike_test_for_parameter(self,
        ...
        if all(medsutil.amath.is_greater_than(cv, test_ranges[1]) or medsutil.amath.is_less_than(cv, test_ranges[0]) for cv in check_values):
    ...
    def _run_spike_test_for_parameter(self, v1: t.Union[amath.AnyNumber, list[amath.AnyNumber], None], v2: amath.AnyNumber, v3: t.Union[
        amath.AnyNumber, list[amath.AnyNumber], None], thresholds: tuple):
        ...
        if self.run_spike_test and thresholds[0] is not None and all(
                medsutil.amath.is_greater_than(cv, thresholds[0]) for cv in check_spikes):
        ...
        if self.run_gradient_test and thresholds[1] is not None and all(
                medsutil.amath.is_greater_than(cv, thresholds[1]) for cv in check_gradients):
