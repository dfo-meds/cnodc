...
from medsutil.ocproc2.codecs.netcdf import NetCDFCommonMapper, MappingInfo
from medsutil.ocproc2 import SingleElement, ParentRecord
import pipeman.programs.glider.ego_convert as ego_convert
...
class GliderEGOMapper(NetCDFCommonMapper):
    ...
    def _on_data_load(self):
        if self._sensor_map is None:
            self._sensor_info, self._sensor_map = ego_convert.ego_sensor_info(
                self._dataset,
                self._data['data_maps']['sensors'],
                self._data['data_maps']['sensor_models'],
    ...
    def _after_element(self, element: SingleElement, minfo: MappingInfo, data: t.Optional[dict[str, t.Any]] = None):
        extra_metadata = {}
        if minfo.source in self._sensor_map:
    ...
    def _isoformat_ego_date(self, value, minfo: MappingInfo):
        if not value:
            return None
        match len(value):
            case 8:
