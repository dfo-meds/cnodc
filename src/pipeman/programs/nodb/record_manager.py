...
from nodb import NODBSourceFile, NODBInstance
...
class NODBRecordManager:
    ...
    def create_completed_entry_from_working_record(self,
                                                   db: NODBInstance,
    ...
    def create_completed_entry(self, db, record: ocproc2.ParentRecord, source_file_uuid: str, received_date: datetime.date, message_idx: int, record_idx: int, memory: dict, original_uuid: str = None):
        check = NODBObservationData.find_by_source_info(
            db, source_file_uuid, received_date, message_idx, record_idx, record.metadata.best('CNODCLevel', coerce=str, default='UNKNOWN'), key_only=True
        )
    ...
    def _prune_metadata(self, db, record: ocproc2.ParentRecord, memory, lookup_key_name, group_name, finder: t.Callable):
