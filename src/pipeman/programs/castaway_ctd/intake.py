...
from pipeman.processing.workflow import FilePayload
from pipeman.processing.payload_worker import FileWorkflowWorker
...
from pipeman.programs.erddap import ErddapController
...
class CastawayIntakeWorker(FileWorkflowWorker):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._upload_target_raw: t.Optional[FilePath] = None
        self._archive_target_raw: t.Optional[FilePath] = None
        self._upload_target_processed: t.Optional[FilePath] = None
        self._archive_target_processed: t.Optional[FilePath] = None
    ...
    def on_start(self):
        ...
        self._upload_target_raw = self.storage.get_filepath(self.get_config("erddap_directory_raw"), halt_flag=self._halt_flag)
        ...
        self._archive_target_raw = self.storage.get_filepath(self.get_config("archive_directory_raw"), halt_flag=self._halt_flag)
        ...
        self._upload_target_processed = self.storage.get_filepath(self.get_config("erddap_directory_processed"), halt_flag=self._halt_flag)
        ...
        self._archive_target_processed = self.storage.get_filepath(self.get_config("archive_directory_processed"), halt_flag=self._halt_flag)
    ...
    def process_payload(self, payload: FilePayload) -> t.Optional[QueueItemResult]:
        ...
        upload_file: t.Optional[FilePath] = None
        archive_file: t.Optional[FilePath] = None
...
class CastawayData:
    ...
    def __init__(self, source_file: pathlib.Path, is_gzipped: bool = False):
        self._is_gzipped = is_gzipped
        # TOOD: may need to ungzip file
    ...
    def _create_netcdf_file(self, netcdf_file: pathlib.Path) -> nc.Dataset:
        ncf = nc.Dataset(netcdf_file, "r", format="NETCDF4")
        try:
